    ...
    <View className="flex-1 bg-white">
      ...
      <View className="bg-background p-6 gap-6">
        <View className="flex-row gap-4 items-center">
          ...
          <View>
            ...
            <Text className="text-gray-400">Feito com amor, servido com paixão</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 p-6 bg-white">
        {loadingProducts ? (
          ...
          <Text className="text-center text-gray-500 mt-10">Nenhum produto encontrado</Text>
        ) : (
          products.map((item) => (
            <TouchableOpacity
              key={item.id}
              ...
              <View className="flex-row items-center mb-6 bg-white p-4 border border-gray-200 rounded-xl gap-4 shadow-sm">
                ...
                <View className="flex-1 gap-2">
                  ...
                  <Text className="text-background text-xl font-semibold mt-1">
                    {typeof item.price === "number" ? item.price.toFixed(2) : item.price} MT
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
